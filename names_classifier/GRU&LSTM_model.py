...
get_ipython().system("unzip -qq 'data.zip'")
DATA_DIR = 'data'
get_ipython().run_line_magic('matplotlib', 'inline')


# ## The code from given RNN tutorial
...
acc = sum/torch.sum(confusion)
print('Accuracy using GRU: {:4f}'.format(acc.item()))
...
def train_lstm(category_tensor, line_tensor):
    ...
    for i in range(line_tensor.size()[0]):
        output, (hidden,hidden1) = lstm(line_tensor[i], (hidden,hidden1))
...
for i in range(18):
  sum = sum + confusion[i][i]
acc = sum/torch.sum(confusion)
print('Accuracy using LSTM: {:4f}'.format(acc.item()))
